...
namespace OOPs.Models
{
    ...
    //eg-
    //here we have create SparrowChild Object and we are calling the constructor of Sparrow class and then the constructor of Bird class.
    ...
    //public class Sparrow : Bird 
    //{ 
    //    //public Sparrow(int a) ----//if write like this  then get CT error because SparrowChild class compiler doesnt pass any parameter
    //   //but when SparrowChild class constructor pass the parameter that time error is not occurs 
    ...
    //Entity class is a class which is used to represent the data in the database.
    //It is also known as POCO (Plain Old CLR Object) class.
    //It is a simple class which has properties and fields to represent the data.
    //It does not have any business logic or methods to manipulate the data.
    //It is used to map the data from the database to the application and vice versa.
    //It is also used to create the database tables using Entity Framework Code First approach.

    //----------------
    //aggregation is a weak association where the lifetime of the contained object
    //is not managed by the container object. In aggregation, if the container object is destroyed,
    //the contained object can still exist. In this case, the service class is not composed of the model class,
    //and if the service class is destroyed, the model class can still exist.
    ...
    //here service class is dependent on the model class
    //because it uses the model class to perform its operations.

    public class DemoService
    {
        private readonly DemoModel model;

        public DemoService()
        {
            model = new DemoModel()
            { 
                Nums = new int[] { 2, 4, 6, 8, 4 }
            };
        }
        //here we can initialize the model with default values if needed
        //its is call Tight coupling because we are creating the model object inside the service class
        //its is also call Composition because the service class is composed of the model class


        public DemoService(DemoModel model)
        {
            this.model = model;
        }
        //here we are using constructor injection to inject the model object into the service class
        //its is call Loose coupling because we are not creating the model object inside the service class
        //its is also call Aggregation because the service class is not composed of the model class, it is just


        public void Display()
        {
            Console.WriteLine("Numbers in the array");
            foreach (var num in model.Nums)
            {
        ...
        public int GetMaxNumber()
        {
            ...
            if (model.Nums.Length == 0)
            {
                ...
                return int.MinValue; // or any other default value
            }
            int max = model.Nums.Max();
            return max;
        ...
        public void SortArray()
        {
            ...
            model.Nums = model.Nums.OrderBy(x => x).ToArray();
            foreach (var num in model.Nums)
            {
                Console.WriteLine(num);
            }
...
//    public override string ToString()
//    {
//        return $"ID :{ID} ,Customre :{Customer} ,Order Item List :{string.Join(" \n",OrderItemList)} ";
//    }
